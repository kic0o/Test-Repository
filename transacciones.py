...
def procesar_transferencia(usuario: str, monto: float, es_banco_externo: bool, balance_actual: float) -> float:
    ...
    MAX_TRANSFERENCIA = 11000.0
    COMISION_EXTERNO = 0.02
    UMBRAL_ALERTA = 5000.0

    if monto > MAX_TRANSFERENCIA:
        raise ValueError(f"La transferencia de {monto} MXN excede el límite de {MAX_TRANSFERENCIA} MXN.")

    comision = monto * COMISION_EXTERNO if es_banco_externo else 0.0
    total_a_cargar = monto + comision

    if total_a_cargar > balance_actual:
        raise ValueError("Saldo insuficiente para realizar la transferencia.")

    nuevo_balance = balance_actual - total_a_cargar

    if monto > UMBRAL_ALERTA:
        enviar_alerta_sms(usuario, monto)

    logging.info(
        f"Transferencia de {monto:.2f} MXN {'externa' if es_banco_externo else 'interna'} procesada. Comisión aplicada: {comision:.2f} MXN. Nuevo balance: {nuevo_balance:.2f} MXN"
    )
